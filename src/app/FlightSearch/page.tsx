        ...
        await Promise.all(
          dateRange.map(async (dateStr) => {
            const res = await fetch(
              `/api/v1/frontoffice/flights?origin=${currentFrom}&destination=${currentTo}&date=${dateStr}&limit=5`,
            );
            const json = await res.json();
            if (json.data && json.data.length > 0) {
              priceMap[dateStr] = Math.min(
                ...json.data.map((f: any) => parseFloat(f.basePrice)),
              );
            } else {
    ...
    const fetchFlights = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/v1/frontoffice/flights?${apiParams.toString()}`,
        );
  ...
  return (
    <>
      <SearchHeader
        ...
        onFromChange={(val) =>
          setLocalSearch((prev) => ({ ...prev, from: val }))
        }
        onToChange={(val) => setLocalSearch((prev) => ({ ...prev, to: val }))}
        onDepartureChange={(val) =>
          setLocalSearch((prev) => ({ ...prev, departure: val }))
        }
        onReturnChange={(val) =>
          setLocalSearch((prev) => ({ ...prev, return: val }))
        }
        onTypeChange={(val) =>
          setLocalSearch((prev) => ({ ...prev, type: val }))
        }
        onCabinChange={(val) =>
          setLocalSearch((prev) => ({ ...prev, cabin: val }))
        }
        onSearch={() => {
                  ...
                  <Card key={flight.id} withBorder radius="md" p="lg">
                    <Group justify="space-between">
                      ...
                      <Stack align="flex-end" gap={0}>
                        <Text fw={900} size="xl">
                          THB {Number(flight.basePrice).toLocaleString()}
                        </Text>
                        <Button
                          color="green"
                          radius="xl"
                          ...
                          onClick={() => {
                            if (
                              ...
                              console.log("Proceeding to Checkout:", {
                                departure: selectedDeparture || flight,
                                return: isSelectingReturn ? flight : null,
                              });
                            }
                          }}
                        >
